...
10. 어제 입고 행을 제외한 다운로드 첫 시트의 A열 배차번호를 `M` 접두사로 변환해 Milkrun 카드와 연결하고 상품 목록을 표시합니다.
...
작업 오류 팝업에서는 상세 traceback과 최근 로그를 확인할 수 있습니다. GitHub 신고 버튼은 민감정보를 가린 전체 보고서를 클립보드에 복사하고 짧은 이슈 작성 주소를 열며, 열린 본문에 `Ctrl+V`로 붙여넣어 신고합니다. 설정의 `업데이트 내역` 버튼에서는 설치된 버전의 변경 기록을 확인할 수 있습니다.
